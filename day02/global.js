...
function changeNumber(num1,num2){
  globalThis.num = num1; //gloabalThis를 통해 전역변수 num을 num1값으로 설정
...
//globalThis는 자바 스크립트 환경에 따라 전역객체에 접근하는 방법이 달라 
// 코드의 이식성이 떨어지는 것을 극복하기 위해
...
console.log(globalThis.number); //undefined 블록스코프 변수이므로 globalThis.number로 설정 안됨, number로 출력시 오류 발생 
